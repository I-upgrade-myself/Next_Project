...
export default function VariantImages() {
   ...
   const [active, setActive] = useState(0)
    return (
        ...
        render={({ field }) => (
          <FormItem>
            <FormLabel>Tags Color</FormLabel>
            <FormControl>
        ...
        <Reorder.Group as="tbody" values={fields} onReorder={(e) => {
          ...
          {fields.map((field,index) => {
            return (
              ...
              <Reorder.Item as="tr" key={field.id} id={field.id} onDragStart={() => setActive(index)} value={field} className={cn(field.url.search('blob:') === 0 ? 'animate-pulse translate-all' : "", 'text-sm font-bold text-muted-foreground hover:text-primary')}>
                <TableCell>{index}</TableCell>
            <TableCell>{field.name}</TableCell>
            <TableCell>{(field.size / (1024 * 1024)).toFixed(2)} MB</TableCell>
            <TableCell><div className="flex items-center justify-center">
                <Image src={field.url} alt={field.name} className="rounded-md" width={72} height={48}></Image>
              </div></TableCell>
              <TableCell>
                <Button variant={'ghost'} onClick={(e) => {e.preventDefault()
                   remove(index)}} className="scale-75"><Trash className="h-4"/></Button>
              </TableCell>
              </Reorder.Item>
            )
          })}
